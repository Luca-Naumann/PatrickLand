...
function initReviewsCarousel() {
    ...
    // Continuous progress thumb — slides smoothly with actual scroll
    // position instead of the dots only ever snapping discretely once
    // a swipe/drag settles on a new card.
    const progressThumb = document.createElement("div");
    progressThumb.className = "review-progress-thumb";
    indicatorsContainer.appendChild(progressThumb);

    function updateProgressThumb() {
        const maxScroll = carousel.scrollWidth - carousel.clientWidth;
        const progress = maxScroll > 0 ? carousel.scrollLeft / maxScroll : 0;
        progressThumb.style.left = `${Math.max(0, Math.min(1, progress)) * 100}%`;
    }
    ...
    // Continuous thumb position — updates on every scroll tick,
    // from any input method (touch, trackpad, mouse-drag, keyboard,
    // or the programmatic scrolls below).
    carousel.addEventListener('scroll', updateProgressThumb, { passive: true });
    updateProgressThumb();
        ...
        (entries) => {
            entries.forEach((entry) => {
                ...
                if (index !== -1 && index !== currentIndex) {
                    ...
                    // Only fire feedback for genuinely user-driven
                    // scrolling (swipe/trackpad/mouse-drag). Button
                    // clicks, keyboard arrows, and autoplay already
                    // trigger their own feedback directly — this
                    // check stops them firing a second time here.
